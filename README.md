# Как проверить работоспособность решения (Комментарий от NikolaevMaxim107v2)
Проверить работоспособность решения можно через командную консоль. Предварительно необходимо через команду cd перейти в папку с исполнительным файлом решения (TestTask.exe).
Для запуска необходимо использовать строку "TestTask.exe *имя_первого_файла*.txt *имя_второго_файла*.txt", можно указать как путь к файлам с текстом, так и только их имена 
(в таком случае они должны в той же директории, что и исполняемый файл!), программа не будет работать, если указано менее или более двух файлов.
